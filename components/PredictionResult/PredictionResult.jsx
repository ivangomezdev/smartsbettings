...
const missingDataLabels = {
  HOME_SEASON_STATISTICS: "Estadísticas de temporada del equipo local no disponibles",
  AWAY_SEASON_STATISTICS: "Estadísticas de temporada del equipo visitante no disponibles",
  H2H: "Historial directo insuficiente o no disponible",
  INJURY_DATA_NOT_AVAILABLE: "Lesiones verificadas no disponibles",
  LINEUP_NOT_CONFIRMED: "Alineación todavía no confirmada",
  ODDS: "Cuotas no disponibles",
  MATCH_STATISTICS: "Estadísticas detalladas de partidos incompletas",
  MATCH_STATISTICS_PARTIAL: "Estadísticas detalladas disponibles solo para parte de la muestra",
  RECENT_CORNERS_PARTIAL: "Muestra reciente de corners incompleta",
  UNSUPPORTED_CRITICAL_CLAIM: "Una afirmación crítica no alcanzó corroboración suficiente",
  STALE_OR_INVALID_EVIDENCE: "Se descartó evidencia antigua o inválida",
  WEB_RESEARCH_BUDGET_EXHAUSTED: "La investigación web alcanzó su límite de búsquedas",
  INSUFFICIENT_SOURCE_QUALITY: "No se encontró una fuente con calidad suficiente",
};
...
function missingDataLabel(value) { return missingDataLabels[value] || String(value).replaceAll("_", " ").toLowerCase(); }
...
export function PredictionResult({ analysis }) {
  ...
  return <article className="prediction-result">
    ...
    {analysis.prediction?.expectedCounts?.total != null ? <p className="prediction-result__edge-warning"><FiInfo /> Conteo esperado: <strong>{decimal(analysis.prediction.expectedCounts.total)}</strong> {analysis.prediction.expectedCounts.statistic === "cards" ? "tarjetas totales" : "corners totales"}.</p> : null}
    ...
    {selections.map((selection) => <section className="prediction-result__odds" key={selection.key}><div><FiTrendingUp /><span>{selection.label}</span><strong>{percent(selection.probability)}</strong></div><span>Cuota justa <strong>{decimal(selection.fairOdds)}</strong></span>{selection.pushProbability != null ? <span>Reembolso por empate <strong>{percent(selection.pushProbability)}</strong></span> : null}<span>Mejor cuota <strong>{decimal(selection.marketOdds)}</strong>{selection.bookmaker ? <small>{selection.bookmaker}</small> : null}</span><span>Edge teórico <strong>{percent(selection.theoreticalEdge)}</strong><em>Experimental</em></span></section>)}
    ...
    <details open><summary><FiBarChart2 /> Últimos 6</summary><div className="prediction-result__forms"><LastSix label={event.homeTeam?.name || "Local"} team={analysis.recentForm?.home} /><LastSix label={event.awayTeam?.name || "Visitante"} team={analysis.recentForm?.away} /></div></details>
    ...
    <details><summary>Información no disponible</summary><p>{explanation.missingDataCommentary}</p><ul>{(analysis.missingData || []).map((item) => <li key={item}>{missingDataLabel(item)}</li>)}</ul></details>
    ...
    <details><summary>Información técnica</summary><p>Modelo: {analysis.model?.version || "—"}</p><p>Router: {analysis.model?.routerVersion || "—"}</p><p>{analysis.llm?.fallbackUsed ? `LLM_FALLBACK_USED · ${analysis.llm?.warning || "provider no disponible"}` : `Explicación: Smartsbettings IA / ${analysis.llm?.model}`}</p></details>
    <footer><span>Modelo: {analysis.model?.version}</span><span>{analysis.llm?.fallbackUsed ? "Explicación determinista" : `Explicado con smartsbettings IA`}</span></footer>
  </article>;
}
