...
export function parsePredictionQuery(input, options = {}) {
  ...
  if (marketResult?.unsupported) {
    errors.push({
      ...
      message: marketResult.reason === "UNSUPPORTED_PERIOD"
        ? "Los mercados de primer tiempo todavía no están soportados. Usa el mercado de partido completo."
        : "El mercado o la línea indicada todavía no está soportado.",
      value: marketResult.raw,
      reason: marketResult.reason || "UNSUPPORTED_MARKET",
    });
  }
  ...
  const market = marketResult && !marketResult.unsupported
    ? {
        ...
        ...(marketResult.teamSide ? { teamSide: marketResult.teamSide } : {}),
        ...(marketResult.statistic ? { statistic: marketResult.statistic } : {}),
      }
    : null;
