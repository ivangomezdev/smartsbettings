...
function lastSixTeam(form, teamId) {
  const rows = (form?.matches || []).slice(0, 6).map((match) => {
    const home = match.result?.venue ? match.result.venue === "home" : String(match.homeTeam?.id) === String(teamId);
...
function normalizedMissing(snapshot, webContext, statsSummary) {
  const normalize = (item) => {
    const section = String(item.section || item.code || item || "")
      .replace(/([a-z0-9])([A-Z])/g, "$1_$2")
      .toLowerCase();
    const reason = String(item.reason || "").toLowerCase();
    if (section.includes("lineup")) return "LINEUP_NOT_CONFIRMED";
    if (section.includes("injur")) return "INJURY_DATA_NOT_AVAILABLE";
    if (section.includes("match_statistics") && reason.includes("partial")) return "MATCH_STATISTICS_PARTIAL";
    if (section.includes("xg")) return "XG_NOT_AVAILABLE";
    if (section.includes("recent")) return "RECENT_FORM_NOT_AVAILABLE";
    return section.toUpperCase().replace(/\W+/g, "_");
  };
...
export function buildExplanationContext({ snapshot, prediction, webContext = snapshot?.enrichment?.web || {} } = {}) {
  ...
  return {
    ...
    model: {
      ...
      expectedCounts: prediction.expectedCounts || null,
    },
