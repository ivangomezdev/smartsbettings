...
export function buildTargets(match) {
  ...
  const totalCorners = Number.isFinite(match.homeCorners) && Number.isFinite(match.awayCorners) ? match.homeCorners + match.awayCorners : null;
  const totalCards = Number.isFinite(match.homeCards) && Number.isFinite(match.awayCards) ? match.homeCards + match.awayCards : null;
  return {
    over_0_5: Number(total > 0.5),
    over_1_5: Number(total > 1.5),
    over_2_5: Number(total > 2.5),
    over_3_5: Number(total > 3.5),
    over_4_5: Number(total > 4.5),
    under_1_5: Number(total < 1.5),
    under_2_5: Number(total < 2.5),
    under_3_5: Number(total < 3.5),
    under_4_5: Number(total < 4.5),
    btts: Number(match.homeGoals > 0 && match.awayGoals > 0),
    btts_no: Number(!(match.homeGoals > 0 && match.awayGoals > 0)),
    ...
    double_chance_1x: Number(match.homeGoals >= match.awayGoals),
    double_chance_x2: Number(match.homeGoals <= match.awayGoals),
    double_chance_12: Number(match.homeGoals !== match.awayGoals),
    draw_no_bet_home: match.homeGoals === match.awayGoals ? null : Number(match.homeGoals > match.awayGoals),
    draw_no_bet_away: match.homeGoals === match.awayGoals ? null : Number(match.homeGoals < match.awayGoals),
    home_over_0_5: Number(match.homeGoals > 0.5),
    home_over_1_5: Number(match.homeGoals > 1.5),
    home_over_2_5: Number(match.homeGoals > 2.5),
    home_under_0_5: Number(match.homeGoals < 0.5),
    home_under_1_5: Number(match.homeGoals < 1.5),
    home_under_2_5: Number(match.homeGoals < 2.5),
    away_over_0_5: Number(match.awayGoals > 0.5),
    away_over_1_5: Number(match.awayGoals > 1.5),
    away_over_2_5: Number(match.awayGoals > 2.5),
    away_under_0_5: Number(match.awayGoals < 0.5),
    away_under_1_5: Number(match.awayGoals < 1.5),
    away_under_2_5: Number(match.awayGoals < 2.5),
    cards_over_3_5: totalCards === null ? null : Number(totalCards > 3.5),
    cards_over_4_5: totalCards === null ? null : Number(totalCards > 4.5),
    cards_over_5_5: totalCards === null ? null : Number(totalCards > 5.5),
    cards_under_3_5: totalCards === null ? null : Number(totalCards < 3.5),
    cards_under_4_5: totalCards === null ? null : Number(totalCards < 4.5),
    cards_under_5_5: totalCards === null ? null : Number(totalCards < 5.5),
    corners_over_8_5: totalCorners === null ? null : Number(totalCorners > 8.5),
    corners_over_9_5: totalCorners === null ? null : Number(totalCorners > 9.5),
    corners_over_10_5: totalCorners === null ? null : Number(totalCorners > 10.5),
    corners_under_8_5: totalCorners === null ? null : Number(totalCorners < 8.5),
    corners_under_9_5: totalCorners === null ? null : Number(totalCorners < 9.5),
    corners_under_10_5: totalCorners === null ? null : Number(totalCorners < 10.5),
  };
}
...
function detailedStatistics(rows) {
  return rows.slice(-10).reverse().flatMap(({ match }) => {
    const hasXg = Number.isFinite(match.homeXg) && Number.isFinite(match.awayXg);
    const hasCorners = Number.isFinite(match.homeCorners) && Number.isFinite(match.awayCorners);
    const hasCards = Number.isFinite(match.homeCards) && Number.isFinite(match.awayCards);
    if (!hasXg && !hasCorners && !hasCards) return [];
    return [{
      fixture: { fixtureId: match.id || match.matchKey, date: match.matchDate },
      teams: [
        { team: { id: match.homeTeamNormalized, name: match.homeTeam }, values: {
          ...(hasXg ? { xg: match.homeXg } : {}),
          ...(hasCorners ? { corners: match.homeCorners } : {}),
          ...(hasCards ? { yellowCards: match.homeCards, redCards: 0 } : {}),
        } },
        { team: { id: match.awayTeamNormalized, name: match.awayTeam }, values: {
          ...(hasXg ? { xg: match.awayXg } : {}),
          ...(hasCorners ? { corners: match.awayCorners } : {}),
          ...(hasCards ? { yellowCards: match.awayCards, redCards: 0 } : {}),
        } },
      ],
    }];
  });
  ...
  return {
    build(match) {
      ...
      const snapshot = {
        ...
        matchStatistics: detailedStatistics(combinedHistory),
